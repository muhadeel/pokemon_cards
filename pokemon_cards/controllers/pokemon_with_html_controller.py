...
pokemons_html_bp = Blueprint('pokemons_with_html', __name__)


@pokemons_html_bp.route('', methods=["GET", "POST"])
def index():
...
@pokemons_html_bp.route('new')
def new():
    return render_template('pokemon/new.html')


@pokemons_html_bp.route('/update/<int:id>', methods=["POST"])
def update(id):
    if request.form:
...
@pokemons_html_bp.route('/delete/<int:id>', methods=["POST"])
def delete(id):
